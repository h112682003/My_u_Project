...
void hr_spo2_avg(void){
	...
	//w = r|0x00;//setting 1 samples avg.
	set_write_3(0xAE,REG_ADDR,w);
	set_write_2(0xAE,REG_ADDR);
	r = set_read_1(0xAF,1);
	}while(r!=w);
...
void hr_spo2_led_pa(void){
	uint8_t data1 = 0x3C;// 0x3C = 60, Current = 60*0.2=12mA
	uint8_t data2 = 0x05;// 0x05 = 5, Current = 5*0.2=1mA
	set_write_3(0xAE,0x0C,data2);//LED1_PA
	set_write_3(0xAE,0x0D,data2);//LED2_PA
	set_write_3(0xAE,0x0E,data2);//RESERVED
	set_write_3(0xAE,0x10,data2);//RESERVED
}
